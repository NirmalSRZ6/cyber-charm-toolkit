// Web Crypto-based hashing + a tiny pure-JS MD5 (since SubtleCrypto doesn't expose MD5).
// All operations are local. Educational only.
...
async function subtle(algo: "SHA-1" | "SHA-256" | "SHA-512", input: string): Promise<string> {
  const data = new TextEncoder().encode(input);
  const buf = await crypto.subtle.digest(algo, data);
  return toHex(buf);
}

export const sha1 = (s: string) => subtle("SHA-1", s);
export const sha256 = (s: string) => subtle("SHA-256", s);
export const sha512 = (s: string) => subtle("SHA-512", s);

// --- MD5 (RFC 1321) — pure JS implementation, for educational hashing demos only.
// Adapted from the public-domain implementation by Joseph Myers.
export function md5(str: string): string {
  ...
  function sb(s: string) {
    ...
    const blks: number[] = new Array(nb * 16).fill(0);
    let i;
    for (i = 0; i < s.length; i++) blks[i >> 2] |= s.charCodeAt(i) << ((i % 4) * 8);
    blks[i >> 2] |= 0x80 << ((i % 4) * 8);
    blks[nb * 16 - 2] = s.length * 8;
    return blks;
  }
...
export async function hashAll(input: string): Promise<Record<HashAlgo, string>> {
  ...
  return { md5: md5(input), sha1: s1, sha256: s256, sha512: s512 };
}

export async function hashOne(algo: HashAlgo, input: string): Promise<string> {
  if (algo === "md5") return md5(input);
  if (algo === "sha1") return sha1(input);
  if (algo === "sha256") return sha256(input);
  return sha512(input);
}
